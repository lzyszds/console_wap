...
export const uploadByPieces = ({ file, pieceSize = 2, success, error, upload, uploading, params }) => {
    // 如果文件传入为空直接 return 返回
    if (!file) return console.error("文件为空")
    let fileMD5 = ''// 总文件列表
    const chunkSize = pieceSize * 1024 * 1024 // 5MB一片
    console.log(`lzy  chunkSize:`, chunkSize)
    const chunkCount = Math.ceil(file.size / chunkSize) // 总片数
    console.log(chunkSize, chunkCount, chunkSize)
    // 获取md5
    const readFileMD5 = () => {
        // 读取视频文件的md5
    ...
    const uploadChunk = (chunkInfo) => {
        ...
        return new Promise((resolver, reject) => {
            ...
            params && Object.keys(params).forEach(key => {
                fetchForm.append(key, params[key])
            })
